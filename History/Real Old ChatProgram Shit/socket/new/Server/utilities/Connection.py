...
class Connection:
	...
	def tryRecv(self):
		readyForRecv = select.select([self.connection], [], [], 2)#time the select should wait to be ready again
		if readyForRecv[0]:
			self.data = str(self.connection.recv(8192), "utf-8")
			if len(self.data) > 0:
				packetid = self.data[:3]
				msg = self.data[3:]
				response = self.handlePacket(packetid, msg)
				return response
		else:
	...
	def handlePacket(self, packetid, msg):
		...
		if packetid == "001":
			...
			response = (packetid, msg, "SUCCESSFULLY_SENT")
		return response
