...
import socketserver, threading, datetime
class Clients:
			...
			for clientInList in fileHelper.readTXTFile("data/", "banList"):
				...
				if self.client_address[0] + "\n" in clientInList:
					...
					if (currTime > float(banTime)):
						...
						print("2")
					logHelper.printAndWriteServerLog("[Server/Info] " + str(self.client_address[0])+ ":" + str(self.client_address[1]) + " is temporary banned on the server. Remaining Time: " +  str(int((float(banTime) - currTime)/60)) + "Minutes")
					self.client.sendall(self.StringToBytes("405[Client/Info] You are temporary banned on this server. Remaining Time: " + str(int((float(banTime) - currTime)/60)) + "Minutes"))
					self.client.close()
					self.appendClient = False
				else:
